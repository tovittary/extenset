...
{
    /// <summary>
    /// Converts the <see cref="string" /> to a new <see cref="Uri" /> object with a relative URI.
    /// </summary>
    ...
    public static Uri ToRelativeUri(this string uriString)
    {
        if (uriString is null) throw new ArgumentNullException(nameof(uriString));

        var trimmed = uriString.Trim().TrimEnd('/');
        return Uri.TryCreate(trimmed, UriKind.Relative, out var uri)
            ? uri
